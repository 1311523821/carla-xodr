...
### 目录名：用相对扫描根的路径是错的，已改成只用文件名

早期版本的路点存档是全局一份 `traces.json`，换加载 FBX 会把上一个场景的线带过来 ——
就是这么串台过。改成"每场景一份"之后，目录名用的是**相对 `--scan-root` 的路径**把 `/`
换成 `__`，理由是"防同名撞车"：`test.fbx` 和 `model/test.fbx` 会挤进同一个 `cache/test/`，
共用一份 GLB 和一份存档。

这个理由本身没错，但代价更大，而且是我没想到的：**目录名成了 scan-root 的函数**。
于是换一种起法，同一份扫描就映射到另一个目录 ——

| 起法 | `~/mindcloud/test_5.fbx` 的目录 |
|---|---|
| `--scan-root ~/mindcloud` | `cache/test_5/` |
| 不传（默认家目录） | `cache/mindcloud__test_5/` |

描的线和标定解全都"看不见"了，**而工具一声不响**：它在新目录里重新烘一份空的
`scene.glb`，看着完全正常。人会以为线丢了，然后重描一遍，磁盘上就留下两份
`traces.json`，谁也不知道哪份是真的。这不是"找不到文件"级别的 bug，是**能让人悄悄
覆盖自己工作**的 bug。

现在的规则：目录名 = FBX 文件名，和 scan-root 完全脱钩；防同名撞车改用目录里一个
`source` 认领文件（记着这个目录属于哪个绝对路径），后来者退到 `cache/test__<哈希前6位>/`。
同一个文件路径永远算出同一个目录。实测：先用 `--scan-root ~/mindcloud` 加载并存一条线，
换 `--scan-root ~` 重启重新加载，目录仍是 `cache/test_5/`，线还在。

**教训**：派生出来的存储路径不能依赖一个"用户每次自己决定"的参数。要么固定成内容/
文件自身的属性，要么就得在参数变化时报警 —— 而这里连变化都检测不到，因为旧目录还在
原地、新目录是凭空长出来的。

### 迁移老存档用搬的，不用复制

加载某场景时若根目录那份老 `traces.json` 的 `scene` 正好是它，就**搬**进场景目录
（`shutil.move`，不是复制）—— 复制会造成"删掉 cache 之后下次加载又复活一份"，等于删不掉。
